...
namespace PictionaryMusicalCliente.VistaModelo.Cuentas
{
    public class VentanaPrincipalVistaModelo : BaseVistaModelo
    {
        ...
        private void ListaAmigosServiceOnListaActualizada(object sender, ListaAmigosActualizadaEventArgs e)
        {
            if (e == null)
            {
                return;
            ...
            if (e.TieneError)
            {
                MostrarMensaje?.Invoke(e.MensajeError ?? Lang.errorTextoServidorNoDisponible);
            }
        }

        private void ActualizarAmigos(IReadOnlyList<string> amigos)
        {
            if (Amigos == null)
            {
                Amigos = new ObservableCollection<string>();
            ...
            string seleccionadoAnterior = AmigoSeleccionado;
            ...
            if (!string.IsNullOrWhiteSpace(seleccionadoAnterior) && amigos != null)
            {
                string coincidencia = amigos
                    .FirstOrDefault(a => string.Equals(a, seleccionadoAnterior, StringComparison.OrdinalIgnoreCase));
                AmigoSeleccionado = coincidencia;
            }
            else if (!string.IsNullOrWhiteSpace(seleccionadoAnterior))
            {
