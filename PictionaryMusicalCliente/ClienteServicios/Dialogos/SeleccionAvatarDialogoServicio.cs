...
namespace PictionaryMusicalCliente.ClienteServicios.Dialogos
{
    ...
    public class SeleccionAvatarDialogoServicio : ISeleccionarAvatarServicio
    {
        ...
        {
            _logger.Info("Iniciando proceso de selecci�n de avatar.");
            ...
            Application.Current.Dispatcher.Invoke(() =>
            {
                try
                {
                    ...
                    {
                        _logger.Info("Avatar seleccionado: ID {0}", avatar?.Id);
                ...
                {
                    _logger.Error("Error XAML al cargar la interfaz de selecci�n de avatar.", ex);
                    finalizacion.TrySetException(
                        new InvalidOperationException(
                            "Error al cargar la interfaz de selecci�n de avatar.",
                            ex));
                }
                catch (InvalidOperationException ex)
                {
                    _logger.Error("Operaci�n inv�lida al mostrar di�logo de avatar.", ex);
