...
namespace PictionaryMusicalCliente.ClienteServicios.Wcf
{
        ...
        PictionaryServidorServicioSalas.ISalasManejadorCallback
    {
            ...
            DTOs.ConfiguracionPartidaDTO configuracion)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _logger.Info("Sala creada exitosamente por '{0}'. " +
                        "Código: {1}", nombreCreador, sala.Codigo);
                    return sala;
                }
        ...
        public async Task<DTOs.SalaDTO> UnirseSalaAsync(string codigoSala, string nombreUsuario)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _logger.Info("Usuario '{0}' se unió a la sala '{1}'.", nombreUsuario, codigoSala);
                    return sala;
                }
                catch (FaultException ex)
                {
                    _logger.Warn("Fallo al unirse a sala '{0}'. Razón: {1}", codigoSala, ex.Message);
                    string mensaje = ErrorServicioAyudante.ObtenerMensaje(
                        ex,
                        Lang.errorTextoErrorProcesarSolicitud);
                    throw new ServicioExcepcion(TipoErrorServicio.FallaServicio, mensaje, ex);
        ...
        public async Task AbandonarSalaAsync(string codigoSala, string nombreUsuario)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _logger.Info("Usuario '{0}' abandonó la sala '{1}'.", nombreUsuario, codigoSala);
                }
                catch (FaultException ex)
                {
            ...
            string nombreJugadorAExpulsar)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _logger.Info("El host '{0}' expulsó a '{1}'" +
                        " de la sala '{2}'.", nombreHost, nombreJugadorAExpulsar, codigoSala);
                }
                catch (FaultException ex)
                {
        ...
        {
            _logger.Info("Callback recibido: '{0}' se unió a la sala " +
                "'{1}'.", nombreJugador, codigoSala);
        ...
        {
            _logger.Info("Callback recibido: '{0}' salió de la sala '{1}'.", nombreJugador, codigoSala);
        ...
        {
            _logger.Info("Callback recibido: '{0}' fue expulsado de la sala " +
                "'{1}'.", nombreJugador, codigoSala);
